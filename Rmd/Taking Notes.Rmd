...
#### 1.a I used Visual Studio 2022 to establish a new database on my local PC: "Coursera_Capstone_Project".

##### 1.b I created a table on the new database for this data: "Cyclistic_divvy_tripdata"

By copying the CSV headers into my editor, and them using them as a basis for the new table, I was able to create a table situated to received the CSV data.
...
#### 2.I then established a connection to the database in RStudio, and ensured the new table was showing.
...
#### 3. With a connection to my local database established in RStudio, I began pulling CSVs into the database.

To accomplish this, I created a SQL script with the `-- !preview conn=conn` at the top. THis step is important in RStudio, as it allows the SQL Script to speak directly with the database, I ran a BULK INSERT in SQL via RStudio's SQL Editor with database connection turned on from the previous step.I created a new line for each CSV so I would retain the original format for each.
...
![VisualStudio2022_ViewData](images/VisualStudio2022_ViewData.jpg)

------------------------------------------------------------------------

##### *Notes about RStudio and SQL:* 

*I found that RStudio (at least with my current settings) would not indicate whether the SQL script was still running. A short hardly-noticable flash would be the only indication the script was complete. Because of this, I frequently found myself trying to run the next script and getting an error that the connection was busy. Because of this, I also couldn't run a query to see if the data was present (because the connection was still busy writing it). Due to this ambiguity, I found that it was easier to wait a few moments for the screen flash, and use Visual Studio 2022 to view the data to see if it had been completed, before moving on to the next line.*

TWO issues inserting 
1. CSV formatting m/d/yyyy hh:mm
2. Duplicate data broke my Primary Key, so I rebuilt the table without it and I'll sort out duplicate data first.

4\. With all the data imported...
